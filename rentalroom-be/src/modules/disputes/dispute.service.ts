...
export class DisputeService {
  ...
  private readonly systemActorId =
  ...
  async resolveDispute(
    ...
    if (
      ...
      dto.approvedAmount > Number(dispute.claimAmount)
    ) {
      throw new BadRequestException(
    ...
    const actorRole =
      adminId === this.systemActorId ? UserRole.SYSTEM : UserRole.ADMIN;
    ...
    const updatedDispute = await this.prisma.$transaction(async (tx) => {
      ...
      if (!updated) {
        throw new NotFoundException('Dispute not found after update');
      }
      ...
      this.stateLogger.logTransitionSafe({
        ...
        metadata: {
          approvedAmount: parseFloat(
            new Decimal(dto.approvedAmount).toString(),
          ),
      ...
      await this.snapshotService.create(
        {
          ...
          metadata: {
            ...
            claimAmount: parseFloat(
              new Decimal(dispute.claimAmount).toString(),
            ),
            reason: dto.reason,
  ...
  private async processDisputeOutcomeTx(
    ...
    if (dispute.status === 'APPROVED' || dispute.status === 'PARTIAL') {
      const approvedAmt = dispute.approvedAmount
        ...
        : 0;
      await tx.disputeRefund.upsert({
        where: { disputeId: dispute.id },
        update: {
          amount: new Decimal(approvedAmt),
          status: 'PENDING',
          reason: dispute.resolutionReason || null,
        },
        create: {
