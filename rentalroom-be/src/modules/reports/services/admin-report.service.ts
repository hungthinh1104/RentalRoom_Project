...
export class AdminReportService {
  ...
  async getAdminOverview(
    ...
    // Implement time-series trends (last n periods)
    const trends: any[] = [];
    const now = new Date();
    ...
    // Calculate dates for the last n months
    const periodsList: string[] = [];
    for (let i = (query.periods || 6) - 1; i >= 0; i--) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
      periodsList.push(formatPeriod(d));
    }

    const startDate = new Date(
      now.getFullYear(),
      now.getMonth() - (query.periods || 6) + 1,
      1,
    );

    // 1. Revenue Trend (Group by Month)
    const revenueTrend = await this.prisma.$queryRawUnsafe<
    ...
    >(
      ...
      FROM payment
      WHERE status = 'COMPLETED' AND payment_date >= $1
      GROUP BY 1
      ORDER BY 1
    `,
      startDate,
    ...
    // 2. New Tenants & Landlords (Group by Month)
    const newTenantsTrend = await this.prisma.$queryRawUnsafe<
    ...
    const newLandlordsTrend = await this.prisma.$queryRawUnsafe<
      Array<{ period: string; count: number }>
    >(
      `
      SELECT 
    ...
    // 3. User Growth (Total Accumulative)
    const totalUserGrowth = await this.prisma.$queryRawUnsafe<
      Array<{ period: string; count: number }>
    >(
      `
      SELECT 
    ...
    // Base total before the period
    const baseTotalUsers = await this.prisma.user.count({
      where: { createdAt: { lt: startDate } },
    });

    // 4. Map results to months
    let runningTotalUsers = baseTotalUsers;

    // Pre-fetch contracts for active calculation
    const relevantContracts = await this.prisma.contract.findMany({
      where: {
        OR: [
    ...
    for (const period of periodsList) {
      // Find metrics for this period
      const rev = revenueTrend.find((r) => r.period === period)?.amount || 0;
      const newT = Number(
        newTenantsTrend.find((r) => r.period === period)?.count || 0,
      );
      const newL = Number(
        newLandlordsTrend.find((r) => r.period === period)?.count || 0,
      );

      // Update running total users
      const newUsersInPeriod = Number(
        totalUserGrowth.find((r) => r.period === period)?.count || 0,
      );
      runningTotalUsers += newUsersInPeriod;

      // Calculate active contracts for this month
      const [year, month] = period.split('-').map(Number);
      const endOfMonth = new Date(year, month, 0); // Last day of month

      const activeC = relevantContracts.filter((c) => {
        return c.startDate <= endOfMonth && c.endDate >= endOfMonth;
      }).length;
      ...
      trends.push({
        ...
        averageOccupancy: 0, // TODO: Complex historical occupancy
      });
    }
