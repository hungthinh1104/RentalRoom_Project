...
const WARDS_FILE = path.resolve(process.cwd(), '../fe-rental-room/public/data/wards.json');
const ALL_WARDS: Array<{ code: string; name: string; fullName: string; slug: string; type: string; provinceCode: string }> = JSON.parse(fs.readFileSync(WARDS_FILE, 'utf8'));
...
// Load street lists (editable in prisma/addresses.json)
const ADDR_FILE = path.resolve(process.cwd(), 'prisma/addresses.json');
let STREET_BY_CITY: Record<string, string[]> = {};
try {
  STREET_BY_CITY = JSON.parse(fs.readFileSync(ADDR_FILE, 'utf8'));
} catch (e) {
  console.warn('⚠️ Could not load prisma/addresses.json, falling back to faker streets');
}
...
async function main() {
  ...
  // Modular parts runner (if --parts is provided)
  const partsArg = args.find((a) => a.startsWith('--parts='))?.split('=')[1];
  const PARTS = partsArg ? partsArg.split(',').map((p) => p.trim()).filter(Boolean) : [];
  const MODULAR = PARTS.length > 0;
  if (MODULAR) {
    console.log('🔀 Modular seed run — parts:', PARTS.length ? PARTS.join(',') : 'all');
    const runAll = PARTS.length === 0 || PARTS.includes('all');
    let usersRes: any = null;
    let props: any[] = [];

    if (runAll || PARTS.includes('users')) {
      const { seedUsers } = nodeRequire('./seeds/users');
      usersRes = await seedUsers(prisma, { DRY_RUN });
    }

    if (runAll || PARTS.includes('properties')) {
      const { seedProperties } = nodeRequire('./seeds/properties');
      props = await seedProperties(prisma, usersRes ? usersRes.landlords : [], { DRY_RUN });
    }

    if (runAll || PARTS.includes('rooms')) {
      const { seedRooms } = nodeRequire('./seeds/rooms');
      await seedRooms(prisma, props || [], { DRY_RUN }, SEED);
    }

    if (runAll || PARTS.includes('popularSearches')) {
      const { seedPopularSearches } = nodeRequire('./seeds/popularSearches');
      await seedPopularSearches(prisma, { DRY_RUN });
    }

    console.log('🔁 Modular seed run complete');
    return;
  }
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (DRY_RUN) {
      ...
      const _tenantAiProfile = {
        id: faker.string.uuid(),
        tenantId: (tenant as any).userId,
        searchHistory: [
          'phòng trọ quận 1',
  ...
  for (const { landlord } of landlords) {
    ...
    for (let i = 0; i < propertyCount; i++) {
      ...
      const property = DRY_RUN ? ({
        ...
        // Ensure uniqueness by adding landlord short id and index
