...
export class ContractPdfService {
     ...
     */
    async generatePdf(contractId: string, requesterUserId: string): Promise<Buffer> {
        ...
        const user = await this.prisma.user.findUnique({ where: { id: requesterUserId } });
        if (!user) {
        ...
        if (!isTenant && !isLandlord && !isAdmin) {
            throw new BadRequestException('You are not authorized to generate this PDF');
        }
        ...
        if (contract.landlord?.bankName && contract.landlord?.bankAccount) {
            ...
            try {
                ...
                this.logger.warn(`Failed to embed QR code for contract ${contractId}: ${(e as Error).message}`);
            }
        }
