...
export class SnapshotsController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve system-wide audit logs with filtering and pagination. Admin only.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve all legal snapshots for a specific entity (CONTRACT, PAYMENT, CONSENT, MAINTENANCE). Admin only.',
  })
  @ApiParam({
    name: 'entityType',
    enum: ['CONTRACT', 'PAYMENT', 'CONSENT', 'MAINTENANCE'],
  ...
  @ApiOperation({
    ...
    description:
      'Verify that snapshot hash has not been tampered with. Returns boolean validation result.',
  })
  @ApiParam({ name: 'id', type: String, description: 'Snapshot UUID' })
  @ApiResponse({
    status: 200,
    description: 'Snapshot verified successfully',
    schema: {
      example: { id: 'uuid', isValid: true, timestamp: '2026-01-07T...' },
    },
  })
