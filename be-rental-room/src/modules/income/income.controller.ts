...
Landlord tự chịu trách nhiệm xác nhận tính chính xác.`,
  ...
  @ApiOperation({
    ...
    description:
      'Real-time projection - not final tax calculation. For reference only.',
  })
  ...
  async getExpenses(@Param('year') year: string, @CurrentUser() user: any) {
    if (!user || !user.id) {
      throw new UnauthorizedException('User not authenticated');
    }
  ...
  async deleteExpense(
    ...
    return this.expenseService.delete(
      id,
      user.id,
  ...
  @ApiOperation({
    ...
    description:
      '⚠️ PERMANENT ACTION: Creates immutable snapshot. Cannot be undone.',
  })
  async closeTaxYear(@Param('year') year: string, @CurrentUser() user: any) {
    if (!user || !user.id) {
      throw new UnauthorizedException('User not authenticated');
    }
    return this.taxYearSummaryService.closeYear(
      user.id,
      parseInt(year),
      user.id,
    );
  ...
  async exportTaxData(@Param('year') year: string, @CurrentUser() user: any) {
    if (!user || !user.id) {
      throw new UnauthorizedException('User not authenticated');
    }
