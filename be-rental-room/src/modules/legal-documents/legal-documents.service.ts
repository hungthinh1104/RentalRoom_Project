...
export class LegalDocumentsService {
    ...
    private async logAudit(
        ...
        this.logger.log(`[AUDIT] ${action} on document ${documentId} by user ${userId}`);
    }
     ...
     */
    async createDocument(dto: CreateDocumentDto, userId: string, ipAddress?: string, userAgent?: string) {
        ...
        if (existing) {
            throw new BadRequestException(`Document with slug "${dto.slug}" already exists`);
        }
        ...
        this.logger.log(`Created legal document: ${document.title} (${document.id})`);
        return document;
    }
     ...
     */
    async findAll(filters?: { type?: string; isActive?: boolean; isPublished?: boolean }) {
     ...
     */
    async softDeleteDocument(id: string, userId: string, reason?: string, ipAddress?: string, userAgent?: string) {
        ...
        this.logger.warn(`SOFT DELETED document: ${id} by user ${userId}. Reason: ${reason || 'Not specified'}`);
    ...
    async publishVersion(
        ...
        this.logger.log(`Published version ${publishedVersion.version} for document ${documentId}`);
