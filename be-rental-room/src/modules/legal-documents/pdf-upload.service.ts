...
export class PdfUploadService {
    ...
    private readonly uploadDir = join(process.cwd(), 'uploads', 'legal-documents');
    ...
    private validatePdf(file: Express.Multer.File): void {
        ...
        if (!this.allowedMimeTypes.includes(file.mimetype)) {
            throw new BadRequestException(`Invalid file type. Only PDF files are allowed. Got: ${file.mimetype}`);
        }
        ...
        this.logger.log(`PDF validation passed: ${file.originalname} (${file.size} bytes)`);
    }
    ...
    async uploadPdf(
        ...
        this.logger.log(`PDF uploaded: ${file.originalname} (${file.size} bytes, hash: ${fileHash})`);
    ...
    async getPdf(attachmentId: string) {
        ...
        const absolutePath = join(this.uploadDir, attachment.filePath.split('/').pop()!);
        ...
        if (calculatedHash !== attachment.fileHash) {
            ...
            throw new BadRequestException('PDF file integrity check failed - file may be corrupted');
        }
    ...
    async verifyPdfIntegrity(attachmentId: string): Promise<boolean> {
        ...
        try {
            ...
            this.logger.error(`Failed to verify PDF integrity: ${attachmentId}`, error);
            return false;
        }
    }
    ...
    async softDeletePdf(attachmentId: string, userId: string, reason?: string) {
        ...
        this.logger.warn(`PDF soft deleted: ${attachmentId} by user ${userId}. Reason: ${reason || 'Not specified'}`);
