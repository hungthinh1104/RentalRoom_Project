...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginInput) => {
    ...
    // Determine a callback URL (either saved or default)
    ...
    // If NextAuth signIn didn't work for some reason, fall back to direct API login
    login(data, {
      onSuccess: (res: AuthResponse) => {
        const role = (res?.user?.role as string)?.toLowerCase();
        const callbackUrl = getCallbackUrl(searchParams, role);
        console.debug('[Login] fallback success, role=', role, 'callbackUrl=', callbackUrl);
        const target = callbackUrl && callbackUrl !== '/login' ? callbackUrl : (role === 'admin' ? '/dashboard/admin' : role === 'landlord' ? '/dashboard/landlord' : '/dashboard/tenant');
        Promise.resolve(router.push(target)).then(() => router.refresh());
      },
    });
  }
