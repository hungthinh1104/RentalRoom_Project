...
export default function ForgotPasswordContent() {
  ...
  if (submitted) {
    return (
      <div className="w-full max-w-lg">
        <div className="bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] rounded-[32px] p-8 md:p-10 shadow-2xl relative overflow-hidden text-foreground">
          <div className="mx-auto w-20 h-20 bg-success/10 rounded-full flex items-center justify-center mb-8 relative">
          ...
          <div className="text-center space-y-3 mb-8">
            ...
            <p className="text-muted-foreground text-base">We have sent password reset instructions to</p>
            <p className="text-lg font-semibold text-foreground">{email}</p>
          </div>

          <div className="p-4 rounded-2xl bg-info/10 border border-info/20 flex items-start gap-3 mb-8">
  ...
  return (
    <div className="bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)] rounded-[32px] p-8 md:p-10 shadow-2xl relative overflow-hidden text-foreground">
      ...
      <div className="mx-auto w-16 h-16 bg-secondary/30 rounded-2xl flex items-center justify-center mb-6 ring-1 ring-input">
        <KeyRound className="w-8 h-8 text-primary" />
      </div>

      <div className="space-y-3 text-center mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Forgot Password?</h1>
        <p className="text-muted-foreground text-base px-4">Don&apos;t worry! Enter your email and we&apos;ll send you reset instructions.</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        {error && (
          <div className="p-4 rounded-2xl bg-destructive/10 border border-destructive/20 flex items-start gap-3 overflow-hidden">
            <AlertCircle className="w-5 h-5 text-destructive flex-shrink-0 mt-0.5" />
            <p className="text-sm text-destructive leading-relaxed font-medium">{error}</p>
          </div>
        )}
        ...
        <div className="text-center pt-2">
          <Link href="/login" className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-2 font-medium transition-colors">
