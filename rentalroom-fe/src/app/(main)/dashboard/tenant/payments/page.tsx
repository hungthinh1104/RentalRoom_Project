...
// Keep a ticking clock to avoid calling Date.now() during render
useEffect(() => {
...
const handleCheckStatus = useCallback((id: string) => {
  ...
  setLastCheckMap((prev) => {
    if (prev[id] && nowLocal - prev[id] < 60000) {
      return prev; // Rate limited
    }

    // Optimistically update timestamp to prevent double click immediately
    const updated = { ...prev, [id]: nowLocal };
    ...
    checkPaymentMutation.mutate(id, {
      onSuccess: (data: { success: boolean }) => {
        if (data.success) {
          ...
          setLastCheckMap((p) => ({ ...p, [id]: Date.now() }));
        }
      },
    });
...
const isRateLimited = now - lastCheck < 60000;
return (
