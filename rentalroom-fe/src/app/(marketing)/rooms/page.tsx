...
export default function RoomsPage() {
  ...
  // Unified Search: use a single hook for both AI and Filters tabs
  const {
    rooms,
    isLoading,
  ...
  } = useAiSearch(searchTab === "ai" ? aiQuery : "", filters, {
    searchMode: "hybrid",
    limit: 12,
    enabled: true,
    page,
  ...
  // Prefetch popular searches to make suggestions instant
  useEffect(() => {
    prefetchPopularSearches?.();
  }, [prefetchPopularSearches]);
  ...
  const totalPages = Math.max(1, Math.ceil((totalResults || 0) / 12));
            ...
            <TabsContent value="ai" className="mt-0 space-y-5">
              ...
              {aiQuery.trim() && !isLoading && (
                <div className="flex items-center gap-2 flex-wrap animate-in fade-in slide-in-from-top-2 duration-300">
                  <Badge
                    variant="outline"
                    className="bg-primary/5 text-primary border-primary/20 px-3 py-1.5 text-sm font-medium"
                  >
