    ...
    const { data: statsData, isLoading: statsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                ...
                const propertiesData = (properties.data as { data: Property[] })?.data || (properties.data as Property[]) || [];
                const contractsData = (contracts.data as { data: Contract[] })?.data || (contracts.data as Contract[]) || [];
                const invoicesData = (invoices.data as { data: Invoice[] })?.data || (invoices.data as Invoice[]) || [];
                ...
                const activeContracts = Array.isArray(contractsData)
                    ? (contractsData as Contract[]).filter((c) => c.status === 'ACTIVE').length
                    : 0;
                ...
                const thisMonthRevenue = Array.isArray(invoicesData)
                    ? (invoicesData as Invoice[])
                        ...
                        .reduce((sum: number, inv) => sum + (inv.totalAmount || 0), 0)
                    : 0;
                ...
                const totalRooms = Array.isArray(propertiesData)
                    ? (propertiesData as Property[]).reduce(
                        (sum: number, p) => sum + (p.rooms?.length || 0),
                        0,
                    )
                    : 0;
                const occupancyRate =
