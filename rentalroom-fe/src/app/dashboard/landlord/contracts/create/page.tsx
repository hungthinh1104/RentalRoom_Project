...
export default function CreateContractPage() {
    ...
    const handleSubmit = async (data: ContractInput) => {
        try {
            ...
            try {
                showSuccess("Đang tạo tài liệu hợp đồng...");
                await api.post(`/contracts/${contract.id}/generate-pdf-async`);
                // Short artificial delay to ensure backend queue picks it up
                await new Promise(resolve => setTimeout(resolve, 2000));
            } catch (pdfError) {
                console.error("PDF generation trigger failed:", pdfError);
            }

            // 3. Navigate to contract payment/details page
