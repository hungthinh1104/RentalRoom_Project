...
import { MapPin, Home, Building2, MoreHorizontal, ArrowRight, Users } from "lucide-react";
        ...
        <>
                            ...
                            <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="w-[160px]">
                                        ...
                                        <DropdownMenuItem onClick={() => router.push(`/dashboard/landlord/properties/${property.id}/edit`)}>
                                            Chỉnh sửa
                                        </DropdownMenuItem>
                                        <DropdownMenuItem onClick={() => router.push(`/dashboard/landlord/properties/${property.id}/rooms`)}>
                                            Quản lý phòng
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                        ...
                        <div className="p-5 space-y-4">
                            <div>
                                <Link href={`/dashboard/landlord/properties/${property.id}`} className="block">
                                    <h3 className="text-lg font-semibold tracking-tight hover:text-primary transition-colors line-clamp-1">
                                        {property.name}
                                    </h3>
                                </Link>
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-border/40">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-medium">
                                        {property.occupiedRooms || 0}/{property.totalRooms || 0}
                                    </span>
                                    <span className="text-xs text-muted-foreground">
                                        ({Math.round(((property.occupiedRooms || 0) / (property.totalRooms || 1)) * 100)}% lấp đầy)
                                    </span>
                                </div>

                                <Button variant="ghost" size="sm" className="h-8 gap-1 hover:text-primary group/btn" asChild>
            ...
            {deleteId && (
                <DeleteConfirmationDialog
                    ...
                    description="Bạn có chắc muốn xóa bất động sản này? Hành động này không thể hoàn tác và sẽ xóa tất cả phòng liên quan."
                    itemName={properties.find(p => p.id === deleteId)?.name || deleteId}
                />
            )}
        </>
    );
